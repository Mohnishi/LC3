"""
This file is for executing Gym acrobot task with LC^3.
"""
...
TS_scale    = 0.001   # posterior reshaping constant for Thompson sampling
...
#----------------------Environment-------------------#
mjenvs = tconstruct(Acrobot, Threads.nthreads());
env = Acrobot()
...
#------------------Strategy Struct-------------------#
env_tconstructor = n -> tconstruct(Acrobot, n)
mppi = MPPIClamp(
...
gt_mppi = MPPIClamp(
            env_tconstructor = n -> tconstruct(Acrobot, n),  #This is for GT-MPPI
            covar = Diagonal(0.2 ^2 * I, size(actionspace(env), 1)),
            lambda = 0.3,
            H = 30,
            K = 256,
...
#------------------Running the algo-------------------#
function ab_lc3(lc3::LC3, plot::Bool; NITER=1000)
    # save data to the following file
    exper = Experiment("log/ab/ab.jlso", overwrite = false)
...
exper, lg = ab_lc3(lc3, true; NITER=1000);
